...
from server.services.screener_service import ScreenerService
...
mode = st.sidebar.radio(
    "Analysis Mode",
    ["📊 Single Stock", "🔍 S&P 100 Screener"],
    index=0
)

st.sidebar.markdown("---")

# Single Stock Mode Controls
...
# ============================================================================
# S&P 100 SCREENER MODE
# ============================================================================
elif mode == "🔍 S&P 100 Screener":
    st.title("🔍 S&P 100 AI Stock Screener")
    st.markdown("Analyze all S&P 100 stocks and discover the top investment opportunities based on AI-powered technical, fundamental, and sentiment analysis.")
    ...
    if 'screener_progress' not in st.session_state:
        st.session_state.screener_progress = 0
    
    # Control buttons
    col_btn1, col_btn2, col_btn3 = st.columns([1, 1, 3])
    with col_btn1:
        if st.button("🚀 Start Scan", disabled=st.session_state.screener_running):
    ...
    with col_btn2:
        if st.button("⏹️ Stop", disabled=not st.session_state.screener_running):
            st.session_state.screener_running = False
            st.rerun()
    ...
    if st.session_state.screener_running:
        ...
        def progress_callback(current, total, result):
            """Update UI as stocks are analyzed"""
            st.session_state.screener_progress = current
            st.session_state.screener_results.append(result)
            
            # Update progress bar
            progress_bar.progress(current / total)
            status_text.text(f"Analyzing: {current}/{total} stocks... Latest: {result['symbol']}")
            ...
            if st.session_state.screener_results:
                ...
                results_container.dataframe(
                    df[['symbol', 'score', 'signal', 'summary', 'sector', 'price']],
                    use_container_width=True
                )
        
        def stop_flag():
            """Check if user requested stop"""
            return not st.session_state.screener_running
    ...
    if st.session_state.screener_results and not st.session_state.screener_running:
        st.markdown("---")
        st.subheader("📊 Top 10 Investment Opportunities")
        
        top_10 = st.session_state.screener_results[:10]
        
        # Display as cards
        for i, stock in enumerate(top_10, 1):
            signal_color = "#00f5d4" if "Buy" in stock['signal'] else "#ff006e" if "Sell" in stock['signal'] else "#facc15"
            
            st.markdown(f"""
            <div style="background-color: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px; border-left: 5px solid {signal_color}; margin-bottom: 15px;">
                <h3 style="margin:0; color:{signal_color};">#{i} {stock['symbol']} - {stock['signal']} (Score: {stock['score']})</h3>
                <p style="color: #aaa; margin: 5px 0;">Sector: {stock.get('sector', 'N/A')} | Price: ${stock.get('price', 0):.2f} | P/E: {stock.get('pe_ratio', 'N/A')}</p>
                <p style="font-size: 0.95em; color: #ddd; margin-top: 10px;">{stock['summary']}</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        df.index += 1
        st.dataframe(
            df[['symbol', 'score', 'signal', 'summary', 'sector', 'price', 'pe_ratio', 'rsi']],
            use_container_width=True,
            height=600
        )
